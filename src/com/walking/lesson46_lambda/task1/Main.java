...
public class Main {
    public static void main(String[] args) {
        ...
        cars.sort(Comparator.comparing(Car -> Car.getIdentifier().getNumber()));
        printList(cars);

        cars.sort(Comparator.comparing(Car::getColor));
        printList(cars);

        cars.sort(Comparator.comparing(Car -> Car.getIdentifier().getYear()));
        printList(cars);

        cars.sort(Comparator.comparing((Car car) -> car.getIdentifier().getNumber())
                .thenComparing((Car car) -> car.getIdentifier().getYear()));
        printList(cars);
    }
