...
//        scanner.close();
    ...
    private Runnable getChangingTask(Car car, Scanner scanner){
            synchronized (cars) {
                System.out.print(CarServicePrintMessages.CAR_PARAMETERS_INDEX_MESSAGE);
                int parameterIndex = scanner.nextInt();
                System.out.print(CarServicePrintMessages.CAR_PARAMETER_MESSAGE);
                var parameter = scanner.next();

                switch (parameterIndex) {
                    case (1): return () -> cars.get(
                            cars.indexOf(car))
                            .getCarIdentificator()
                            .setPlateNumber(parameter);
                    case (2): return () -> cars.get(
                            cars.indexOf(car))
                            .getCarIdentificator()
                            .setReleaseYear(
                                    Integer.parseInt(parameter));
                    case (3): return () -> cars.get(
                            cars.indexOf(car))
                            .setColor(parameter);
